...
# -------------------------------
# FUNCTIONS (PHẢI ĐẶT CÙNG CẤP, KHÔNG THỤT)
# -------------------------------
...
# BUTTON CHUẨN HÓA
# -------------------------------
if st.button("🔄 Chuẩn hóa dữ liệu từ paste"):
    df_raw = parse_paste_to_df(paste_text)

    if df_raw is None:
        st.error("Không đọc được dữ liệu. Hãy paste từ Excel.")
    else:
...
if "est_table" in st.session_state:
    ...
    row_to_delete = st.number_input(
        "Nhập TT cần xóa",
        min_value=1,
        max_value=len(st.session_state["est_table"]),
        step=1
    )

    if st.button("Xóa dòng"):
        df = st.session_state["est_table"]
        df = df[df["TT"] != row_to_delete]
        df["TT"] = range(1, len(df) + 1)
        st.session_state["est_table"] = df
        ...
        if db.empty:
            ...
            if len(db_cols) < 6:
                ...
                for _, row in est.iterrows():
                    ...
                    # 👇 QUAN TRỌNG: xử lý unit + quantity cho cả 2 mode
                    if estimation_file is not None:
                        unit = row[est_cols[3]]
                        qty_value = row[est_cols[4]]
                    else:
                        unit = row.get("Đơn vị", "")
                        qty_value = row.get("quantity", 0)
