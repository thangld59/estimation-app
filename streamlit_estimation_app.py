...
if estimation_file and price_list_files:
    ...
    if "Quantity" in display_df.columns:
        display_df["Quantity"] = pd.to_numeric(display_df["Quantity"], errors="coerce").fillna(0).map(lambda x: f"{int(x):,}")
    ...
    import io
    buffer = io.BytesIO()
    # Ensure unmatched_df is defined
    if "unmatched_df" not in locals():
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ...
        if isinstance(unmatched_df, pd.DataFrame) and not unmatched_df.empty:
